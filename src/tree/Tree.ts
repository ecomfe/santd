...
export function traverseNodesKey(root: I.TBase[] = [], callback: (args0: string, args1: I.TBase) => boolean) {
    function processNode(node: I.TBase) {
...
function traverseNodesKeyV(root: I.TBase[] = [], callback: (args0: string, args1: I.TBase) => boolean) {
    function processNode(node: I.TBase) {
        const key = node.key;

        if (callback(key, node) !== false) {
            traverseNodesKeyV(node.children as unknown as I.TBase[], callback);
        }
    }
...
function toggleArrayData(check: boolean, data: I.keysArray = [], key: string) {
    data = data.concat();
...
function checkNodeExpandedV(node: Base) {
    // 如果一个节点的父节点是收起的，那这个节点也是收起的
    let isExpanded = true;
    let parent = node.parent as I.TBase;
    while (parent) {
        if (!parent.expanded) {
            isExpanded = false;
            break;
        } else {
            parent = parent.parent as I.TBase;
        }
    }
    return isExpanded;
}

export default class Tree extends Base<I.State, I.Props, I.Computed> {
    static components =  {
    ...
    static computed = {
        ...
        // 虚拟滚动中，展开的节点的列表
        expandedNodesV(this: Tree) {
            const flatNodes = this.data.get('flatNodes');
            return flatNodes && flatNodes.filter((node: Base) => checkNodeExpandedV(node));
        },
        // 虚拟滚动中，渲染的节点的数量
        visibleCountV(this: Tree) {
            return Math.ceil(+this.data.get('height') / NODE_HEIGHT_V);
        },
        // 虚拟滚动中，渲染的节点的信息
        visibleNodeV(this: Tree) {
    ...
    inited(): void {
        ...
        this.data.set('allCheckedKeys', isPlainObject(checkedKeys) ? (checkedKeys as I.objKeys).checked : checkedKeys);
        this.data.set('allHalfCheckedKeys', isPlainObject(checkedKeys) ? (checkedKeys as I.objKeys).halfChecked : []);
        ...
        this.watch('checkedKeys', val => {
            ...
            // 如果有父子关联，拿到整个链上的数据
            if (!checkStrictly) {
                let allKeys = this.getAllCheckedKeys(checkedKeys.concat());
    ...
    getAllCheckedKeys(checkedKeys: I.keysArray = []): I.allKeysType {
        const checkStrictly = this.data.get('checkStrictly');
        let checkedKey: string;
        let allKeys: I.allKeysType = {
            checkedKeys: [],
            halfCheckedKeys: []
        };
        while (checkedKeys.length) {
            ...
            if (!allKeys.checkedKeys.includes(checkedKey)) {
                const treeNodeDataV: I.TBase = this.data.get('isVirtual')
                    && this.data.get('flatNodes').find((node: I.TBase) => node.key === checkedKey);
    ...
    getCheckedNodes(treeNodes: I.TBase[], keys: I.keysArray): I.TBase[] {
        let nodes: I.TBase[] = [];
    ...
    getChangedCheckedKeys(key: string, isCheck: boolean, checkedKeys: I.keysArray = [], halfCheckedKeys: I.keysArray = [] = [], checkStrictly: boolean, treeNodeDataV: I.TBase): I.allKeysType {
        ...
        checkedNodes.forEach(node => {
            ...
            if (!checkStrictly) {
                if (!this.data.get('isVirtual')) {
                    let parent = node.parentComponent as I.TBase | this;
                    // 找到后不断遍历父节点，把需要改变状态的父节点的key都拿到
                    while (parent && parent !== this && !(parent as I.TBase).data.get('disabled')) {
                        // 先过滤掉是disabled状态的节点
                        let treeNodes = parent.treeNodes.filter(node => !this.data.get('disabled') && !node.data.get('disabled'));
                        const parentKey = (parent as I.TBase).data.get('key');
                        ...
                        parent = parent.parentComponent as I.TBase | this;
                    }
                    ...
                    let parent = treeNodeDataV.parent as I.TBase;
                    while (parent && !parent.disabled) {
                        let treeNodes = (parent.children as unknown as I.TBase[]).filter(node => !this.data.get('disabled') && !node.disabled);
                    ...
                    if (treeNodeDataV.children && (treeNodeDataV.children as unknown as I.TBase[]).length) {
                        const disabled = this.data.get('disabled') || node.disabled;
                        !disabled && traverseNodesKeyV((treeNodeDataV.children as unknown as I.TBase[]), (key, node) => {
    ...
    attached(): void {
        ...
        !checkStrictly && this.nextTick(() => {
            let allKeys = this.getAllCheckedKeys(this.data.get('allCheckedKeys').concat());
    ...
    autoExpand(): void {
        ...
        while (expandComponents.length) {
            let expand = expandComponents.pop() as Base | this;
            while (expand !== this) {
                expand = expand.parentComponent as Base;
        ...
        this.data.set('hiddenKeys', hiddenKeys.filter((key: string) => !expandedKeys.includes(key)));
    };
    ...
    // 找所有展开的节点
    findExpandComponents(): I.TBase[] {
        let expandComponents: I.TBase[] = [];
    ...
    scrollTimeoutIDV: NodeJS.Timeout | null = null;

    scrollEventV(): void {
        this.data.set('isScrollingV', true);

        // 虚拟滚动中，当前的滚动位置
        const scrollTop = (this.ref(`${prefixClsV}-container`) as unknown as HTMLElement).scrollTop;
    ...
    flattenTreeData(treeData: I.TBase[]) {
        const flatNodes: I.TBase[] = [];
        let treeDepth = 0;
        let currentDepth = 0;
        let nodeCount = 0;
        const expandedKeys = this.data.get('expandedKeys');

        const dig = (treeData: I.TBase[], parent?: I.TBase) =>
            treeData.map((treeNode, index) => {
                currentDepth++;
                if (!treeNode.children || !(treeNode.children as unknown as I.TBase[]).length) {
                    treeDepth = Math.max(treeDepth, currentDepth);
                    currentDepth = 0;
                }

                const flatNode: I.TBase = Object.assign({}, treeNode, {
                    parent,
                    children: null,
                    index: nodeCount++,
                ...
                if (this.data.get('defaultExpandAll') && treeNode.children) {
                    ...
                    if (expanded) {
                        ...
                        while (loopedParent) {
                            ...
                            loopedParent = loopedParent.parent as I.TBase;
                        }
                    }
                }
                ...
                (flatNode.children as unknown as I.TBase[]) = dig((treeNode.children as unknown as I.TBase[]) || [], flatNode);
    ...
    handleSelect(key: string, info: I.clickTreeNodePayloadValueType) {
    ...
    static messages = {
        // 拿到子节点，便于后面传递数据
        santd_tree_addTreeNode(this: Tree, payload: I.addTreeNodePayloadType) {
            this.treeNodes.push(payload.value);
        },
        // 点击节点的处理
        santd_tree_clickTreeNode(this: Tree, payload: I.clickTreeNodePayloadType) {
            this.handleSelect(payload.value.key, payload.value);
        },
        // check节点的处理
        santd_tree_checkTreeNode(this: Tree, payload: I.checkTreeNodePayloadType) {
        ...
        // 展开节点的处理
        santd_tree_expandTreeNode(this: Tree, payload: I.expandTreeNodePayloadType) {
            if (this.data.get('isVirtual')) {
                const flatNodesCopy: I.TBase[] = Array.from(this.data.get('flatNodes'));
        ...
        // 展开所有节点
        santd_tree_expandAll(this: Tree, payload: I.expandAllPayloadType) {
        ...
        // 加载数据完成时候的处理
        santd_tree_dataLoaded(this: Tree, payload: I.dataLoadedPayloadType) {
