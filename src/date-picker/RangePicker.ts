/**
 * @file Santd range picker createpicker file
 * @author mayihui@baidu.com
 **/
import Base from 'santd/base';
import RangeCalendar from '../calendar/src/RangeCalendar';
import dayjs from 'dayjs';
import {classCreator} from '../core/util';
import Trigger from '../core/trigger';
import Placements from '../calendar/src/placements';
import Icon from '../icon';
import {
    RangerPickerProps as Props,
    RangerPickerState as State,
    RangerPickerComputed as Computed,
    RangeSelectEvent,
    RangeChangeEvent,
    RangePanelChangeEvent,
    RangeValue
} from './interface';

const prefixCls = classCreator('calendar')();

function getShowDateFromValue(value: RangeValue, mode?: Props['mode']): RangeValue | void {
    const [start, end] = value;
    // value could be an empty array, then we should not reset showDate
    if (!start && !end) {
        return;
    }
    if (mode && mode[0] === 'month') {
        return [start, end] as Exclude<RangeValue, []>;
    }
    const newEnd = end && end.isSame(start, 'month') ? end.add(1, 'month') : end;
    return [start, newEnd] as Exclude<RangeValue, []>;
}

function isEmptyArray(arr: any[]) {
    if (Array.isArray(arr)) {
        return arr.length === 0 || arr.every(i => !i);
    }
    return false;
}

function isCauseHasKeys(value: RangeSelectEvent['cause'] | {}): value is NonNullable<RangeSelectEvent['cause']> {
    if (value) {
        return Object.keys(value).length > 0;
    }
    return false;
}

function pickerValueAdapter(value: dayjs.Dayjs | RangeValue | undefined): RangeValue | void {
    if (!value) {
        return;
    }
    if (Array.isArray(value)) {
        return value;
    }
    return [value, value.add(1, 'month')];
}

export default class RangePicker extends Base<State, Props, Computed> {
    static computed: Computed = {
        displayStartValue(this: RangePicker) {
            const value = this.data.get('value');
            const format = this.data.get('format');
            return value && value[0] && value[0].format(format);
        },
        displayEndValue(this: RangePicker) {
            const value = this.data.get('value');
            const format = this.data.get('format');
            return value && value[1] && value[1].format(format);
        },
        calendarClasses(this: RangePicker) {
            let classArr = [];
            const showTime = this.data.get('showTime');
            const ranges = this.data.get('ranges');

            showTime && classArr.push(`${prefixCls}-time`);
            ranges && classArr.push(`${prefixCls}-range-with-ranges`);

            return classArr.join(' ');
        }
    }

    static components = {
        's-icon': Icon,
        's-trigger': Trigger,
        's-rangecalendar': RangeCalendar
    }

    static template = `<span
        id="{{id}}"
        class="{{pickerClass}}"
        tabIndex="{{disabled ? -1 : 0}}"
    >
        <s-trigger
            prefixCls="${prefixCls}-picker-container"
            popupTransitionName="{{transitionName}}"
            dropdownClassName="{{dropdownClassName}}"
            getCalendarContainer="{{getCalendarContainer}}"
            visible="{{open}}"
            action="{{disabled ? [] : trigger}}"
            builtinPlacements="{{placements}}"
            popupPlacement="{{popupPlacement}}"
            destroyPopupOnHide="{{true}}"
            on-visibleChange="handleOpenChange"
        >
            <s-rangecalendar
                prefixCls="${prefixCls}"
                style="{{popupStyle}}"
                slot="popup"
                separator="{{separator}}"
                format="{{format}}"
                class="{{calendarClasses}}"
                showTime="{{showTime}}"
                disabledDate="{{disabledDate}}"
                disabledTime="{{disabledTime}}"
                dateInputPlaceholder="{{dateInputPlaceholder || customLocale.lang.rangePlaceholder}}"
                value="{{showDate}}"
                inputReadOnly="{{inputReadOnly}}"
                selectedValue="{{value}}"
                hoverValue="{{hoverValue}}"
                showToday="{{showToday}}"
                mode="{{mode}}"
                locale="{{customLocale.lang}}"
                localeCode="{{customLocaleCode}}"
                hasExtraFooter="{{hasExtraFooter}}"
                hasDateRender="{{hasDateRender}}"
                ranges="{{ranges}}"
                on-select="handleChange"
                on-panelChange="handlePanelChange"
                on-ok="handleOk"
                on-calendarChange="handleCalendarChange"
            >
                <slot name="renderExtraFooter" slot="renderExtraFooter" />
                <slot name="dateRender" slot="dateRender" var-current="{{current}}" />
            </s-rangecalendar>
            <div class="{{pickerInputClass}}">
                <input
                    disabled="{{disabled}}"
                    readOnly
                    value="{{displayStartValue}}"
                    placeholder="{{placeholder && placeholder[0] || customLocale.lang.rangePlaceholder[0]}}"
                    class="${prefixCls}-range-picker-input"
                    tabIndex="-1"
                    style="{{inputStyle}}"
                />
                <span class="${prefixCls}-range-picker-separator">{{separator}}</span>
                <input
                    disabled="{{disabled}}"
                    readOnly
                    value="{{displayEndValue}}"
                    placeholder="{{placeholder && placeholder[1] || customLocale.lang.rangePlaceholder[1]}}"
                    class="${prefixCls}-range-picker-input"
                    tabIndex="-1"
                    style="{{inputStyle}}"
                    s-ref="input"
                />
                <s-icon
                    s-if="!disabled && allowClear && value && (value[0] || value[1])"
                    type="close-circle"
                    class="${prefixCls}-picker-clear"
                    theme="filled"
                    on-click="handleClearSelection"
                />
                <span class="{{prefixCls}}-picker-icon" s-if="hasSuffixIcon">
                    <slot name="suffixIcon" />
                </span>
                <s-icon class="${prefixCls}-picker-icon" type="calendar" s-else />
            </div>
        </s-trigger>
    </span>`

    initData(): State {
        return {
            allowClear: true,
            showToday: false,
            separator: '~',
            hoverValue: [],
            disabledTime() {},
            trigger: 'click',
            placements: Placements,
            popupPlacement: 'bottomLeft'
        };
    }
    inited() {
        const value = this.data.get('value');
        const defaultValue = this.data.get('defaultValue');
        const pickerValue = !value || isEmptyArray(value) ? this.data.get('defaultPickerValue') : value;
        let align = this.data.get('align');
        let placements = this.data.get('placements');
        let popupPlacement = this.data.get('popupPlacement');
        if (align && typeof align === 'object') {
            placements[popupPlacement] = {...placements[popupPlacement], ...align};
        }
        const newValue = pickerValueAdapter(pickerValue || dayjs());

        this.data.set('value', value || defaultValue || []);
        newValue && this.data.set('showDate', newValue);
        this.data.set('hasExtraFooter', !!this.sourceSlots.named.renderExtraFooter);
        this.data.set('hasDateRender', !!this.sourceSlots.named.dateRender);
        this.data.set('hasSuffixIcon', !!this.sourceSlots.named.suffixIcon);
    }
    handleOpenChange(open: boolean) {
        if (open === false) {
            this.data.set('hoverValue', []);
        }
        this.data.set('open', open);
        this.fire('openChange', open);
    }
    handleClearSelection(e: Event) {
        e.preventDefault();
        e.stopPropagation();

        this.data.set('value', []);
        this.handleChange({selectedValue: []});
        this.dispatch('UI:form-item-interact', {fieldValue: '', type: 'change', e});
    }
    handleChange(data: RangeSelectEvent) {
        const selectedValue = data.selectedValue;
        const cause = data.cause || {};

        this.data.set('value', selectedValue);
        this.data.set('selectedValue', selectedValue);

        const format = this.data.get('format');
        const [start, end] = selectedValue;

        const showDate = getShowDateFromValue(selectedValue);
        if (showDate) {
            this.data.set('showDate', showDate);
        }

        this.fire('change', {
            date: selectedValue,
            dateString: [start && start.format(format) || '', end && end.format(format) || '']
        });
        this.dispatch('UI:form-item-interact', {fieldValue: selectedValue, type: 'change'});

        const source = isCauseHasKeys(cause) ? cause.source : '';

        if (source === 'keyboard'
            || source === 'dateInputSelect'
            || (!this.data.get('showTime') && source !== 'dateInput')
            || source === 'todayButton'
        ) {
            this.handleOpenChange(false);
        }
    }
    setValue(value: RangeSelectEvent, hidePanel: boolean) {
        this.handleChange(value);
        if ((hidePanel || !this.data.get('showTime'))) {
            this.data.set('open', false);
        }
    }
    handleRangeClick(value: RangeSelectEvent | (() => RangeSelectEvent)) {
        if (typeof value === 'function') {
            value = value();
        }

        this.setValue(value, true);

        this.fire('ok', value);
        this.fire('openChange', false);
    }
    handleRangeMouseLeave() {
        if (this.data.get('open')) {
            this.data.set('hoverValue', []);
        }
    }
    handleOk(value: NonNullable<Props['value']>) {
        this.fire('ok', value);
        this.handleOpenChange(false);
    }
    handlePanelChange(params: RangePanelChangeEvent) {
        this.fire('panelChange', params);
    }
    handleCalendarChange(value: RangeChangeEvent) {
        this.fire('calendarChange', value);
    }
};

export type TRangePicker = typeof RangePicker;
