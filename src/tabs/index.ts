...
export default class Tabs extends Base<I.State, I.Props, I.Computed> {
    ...
    static messages: I.Messages = {
        ...
        santd_tabs_removeTabPane(payload) {
            this.tabPanes.forEach((tabPane: {data: Data}, index: number) => {
    ...
    getNextActiveKey(next: boolean) {
        ...
        const arr = tabBarData.filter((item: {disabled: boolean}) => !item.disabled);
        const len = arr.length;
        let ret = __activeKey__;
        let currentIndex = arr.findIndex((item: {key: string}) => item.key === __activeKey__);
    ...
    handleTabClick(payload: { key: string; }) {
        this.fire('tabClick', payload.key);
        this.setActiveKey(payload.key);
    };

    handleNavKeyDown(e: { keyCode: number; preventDefault: () => void; }) {
    ...
    setActiveKey(key: string) {
        if (this.data.get('__activeKey__') !== key) {
    ...
    updateTab() {
        ...
        let tabBarData: any[] = [];
        this.tabPanes.forEach((tabPane: {data: Data}) => {
    ...
    attached() {
        this.updateTab();
        this.watch('tabPosition', () => {
            this.nextTick(() => {
                this.updateTab();
            });
        });
        this.watch('activeKey', (val: string) => this.setActiveKey(val));
    };
