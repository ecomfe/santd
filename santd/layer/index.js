...
const returnFalse = e => e.stopPropagation();
...
export default defineComponent({
    template: `
        <template>
            <div s-ref="layer"
                 s-if="open"
                 s-transition="$fxOpacity"
                 class="${cc()}"
                 style="position:absolute;{{layerStyle}}"><slot/></div>
        </template>
    `,
    $fxOpacity: opacity(8),
    initData() {
    ...
    inited() {
        ...
        this.scrollHandler = followScroll ? this.selfPosition(true) : null;
    ...
    bindLayerEvents() {
        const layout = this.data.get('target');
        let selector = document.querySelectorAll(layout)[0] || window;
        if (this.autoHideHandler) {
            document.addEventListener('mousedown', this.autoHideHandler);
            this.el.addEventListener('mousedown', returnFalse);
            if (!this.scrollHandler) {
                selector.addEventListener('scroll', this.autoHideHandler);
            }
            ...
            if (autoHideExceptParent && pc && pc.el) {
                pc.el.addEventListener('mousedown', returnFalse);
            }
        }

        if (this.scrollHandler) {
            selector.addEventListener('scroll', this.scrollHandler);
        }
    },
    unbindLayerEvents() {
        const layout = this.data.get('target');
        let selector = document.querySelectorAll(layout)[0] || window;
        if (this.autoHideHandler) {
            document.removeEventListener('mousedown', this.autoHideHandler);
            this.el.removeEventListener('mousedown', returnFalse);
            if (!this.scrollHandler) {
                selector.removeEventListener('scroll', this.autoHideHandler);
            }

            const pc = this.parentComponent;
            const autoHideExceptParent = this.data.get('autoHideExceptParent');
            if (autoHideExceptParent && pc && pc.el) {
                pc.el.removeEventListener('mousedown', returnFalse);
            }
        }

        if (this.scrollHandler) {
            selector.removeEventListener('scroll', this.scrollHandler);
        }
    },
